 ...
 * @returns {Charater[][]}
 */
function style (ctx, forceSingleColorEmoji = false, trimLines = false) {
    // remove EOF
    if (ctx.sequences[ctx.sequences.length - 1].text[0] === 0) {
        ctx.sequences = ctx.sequences.slice(0, -1);

        if (ctx.sequences[ctx.sequences.length - 1].text[0] === '\n'.codePointAt(0)) {
            if (ctx.sequences[ctx.sequences.length - 2].text[0] === '\r'.codePointAt(0)) {
    ...
    // remove extra lines
    while (trimLines && ctx.sequences[ctx.sequences.length - 1].text[0] === '\n'.codePointAt(0)) {
        if (ctx.sequences[ctx.sequences.length - 2].text[0] === '\r'.codePointAt(0)) {
            ctx.sequences = ctx.sequences.slice(0, -2)
        } else {
            ctx.sequences = ctx.sequences.slice(0, -1);
