...
const b2uTable = /** @type {{[key: string]:string}} */__webpack_require__(4);
...
const b2u = __webpack_require__(0);
/**
    ...
    while (tile = sequences.shift()) {
        ...
        if (b2u(tile.text).charCodeAt(0) >= 128) {
...

            {
                ...
                for (let current = tile.sequence[ptr]; current.type !== 'text'; current = tile.sequence[++ptr]) {
...
function render(canvas, lines, fontSize = 16, fontName = "MingLiU") {
    ...
    const screenWidth = lines.map(line => line.map(c => c.width).reduce((p, c) => p + c, 0)).reduce((p, c) => c > p ? c : p, 0) * fontSize / 2;
    ...
    for (let i = 0; i < lines.length; i++) {
        for (let j = 0; j < lines[i].length; j++) {
            ...
            ctx.translate(cursor.x * fontSize / 2, cursor.y * fontSize);

            if (lines[i][j].width === 1) {
                drawSingleWidth(ctx, lines[i][j].char, fontSize / 2, fontSize, lines[i][j].styles[0].front, lines[i][j].styles[0].back);

                cursor.x += 1;
            } else {
                drawDoubleWidth(ctx, lines[i][j].char, fontSize, fontSize, lines[i][j].styles[0].front, lines[i][j].styles[0].back, lines[i][j].styles[1].front, lines[i][j].styles[1].back);
